...
Предыдущая статья цикла [J для смертных. Введение](j_for_mortals_1.md)
...
Интересно заметить, что тацитное программирование было описано Бэкусом еще до APL и реализовано им в языке FP. Среди современных языков, поддерживающих такой подход, (кроме, естественно, J) можно назвать Форт и другие конкатенативные языки, а также Haskell (за счет point-free подхода).  
...
### Союзы для последовательных вычислений
...
В J весьма своеобразный способ композиции функций - если указаны два подряд идущих глагола, то это не означает, что они будут применяться последовательно. У них будет свой специальный порядок вызова отдельно для диадного и для монадного вызовов. Такая композиция для двух глаголов называется хуком(крюком). Определим глагол v, который является хук-вызовом глаголов f и g:  
...
Можно было бы предположить, что три подряд идущих глагола являются хуком от хука. Однако это не так – вместо этого реализуется форк-композиция (т.н. «вилка»). Так, монадный вызов форка  
...
Следующая статья цикла [J для смертных. Массивы](j_for_mortals_3.md)
