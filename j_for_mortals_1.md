...
Отличительными особенности языка программирования J:

*   Векторная арифметика.
*   Лаконичность синтаксиса.
*   Обширная стандартная библиотека.
*   Библиотека для рисования 2d графиков и 3d поверхностей.
...
В данном примере определяется новая функция (на языке J это называется «глаголом») «mean», которая рассчитывает среднее значение в массиве чисел. Символом «>» будем предварять вычисляемые выражения. Набор символов «+/%#» дословно означает следующую последовательность действий:
...
Одной из самых сильных сторон J является его графическая подсистема. С примерами можно ознакомиться по ссылке [http://www.jsoftware.com/jwiki/Studio/Gallery](http://www.jsoftware.com/jwiki/Studio/Gallery "http://www.jsoftware.com/jwiki/Studio/Gallery")
...
Прямым предком J является язык APL (расшифровывается как «A Programming Language»). Более того, автором и первого и второго является Кеннет Айверсон. Истории из жизни Кеннета (т.н. «кенекдоты») можно прочитать по ссылке [http://keiapl.info/anec/](http://keiapl.info/anec/ "http://keiapl.info/anec/"). Эпиграфы к настоящей статье взяты именно оттуда. 
...
Следующая статья цикла [J для смертных. Тацитное программирование](j_for_mortals_2.md)
