...
В данном примере определяется новая функция (на языке J это называется «глаголом») «mean», которая рассчитывает среднее значение в массиве чисел. Символом «>» в статьях этого цикла будем предварять вычисляемые выражения. Набор символов «+/%#» дословно означает следующую последовательность действий:
